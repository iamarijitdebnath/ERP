    ...
    <section class="bg-gray-50">
        <div class="mx-auto">
            <div class="flex justify-between mb-4">
                <h3 class="text-2xl font-medium font-sans">{{ isset($department) ? 'Edit Department' : 'Add Department' }}</h3>
                <div class="flex items-center gap-2">
                    <a href="{{ route('hrms.department.index') }}" 
                    ...
                    @if(isset($department))
                        <form id="data_delete" action="{{ route('hrms.department.delete') }}" method="POST" class="contents">
                            @csrf
            ...
            <div class="relative overflow-hidden bg-white border border-gray-200 p-4 sm:rounded-lg">
                <form id="department-form" action="{{ isset($department) ? route('hrms.department.update', ['department' => $department->id]) : route('hrms.department.store') }}" method="POST" autocomplete="off">
                    @csrf
                    @if(isset($department))
                        @method('PUT')
                    @endif
...
<div id="popup-modal" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-lg shadow border border-gray-200 p-6 text-center">
            <h3 class="mb-5 text-lg font-normal text-gray-700">Are you sure you want to delete this department?</h3>
