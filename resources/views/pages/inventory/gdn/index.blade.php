...
@section('title', 'Goods Delivery Note (GDN)')

@section('content')
    <section class="bg-gray-50">
        <div class="mx-auto ">
            <div class="block md:flex justify-between mb-4 items-center">
                <h3 class="text-2xl font-medium font-sans">Goods Delivery Note (GDN)</h3>
                    ...
                    <table id="data_table" class="w-full text-sm text-left text-gray-500">
                        <thead class="text-xs text-gray-700 uppercase bg-blue-100">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-3">From Warehouse</th>
        ...
        const printData = (data) => {
            ...
            if(data.length > 0) {
                data.map((row, index) => {
                    ...
                    let fromWarehouseName = transaction.from_warehouse ? transaction.from_warehouse.name : '-';
                    let itemsCount = transaction.items ? transaction.items.length : 0;
                    
                    let issuedByName = row.issued_by ? (row.issued_by.first_name + ' ' + row.issued_by.last_name) : '-';
                    if (!row.issued_by && row.issuedBy) {
                         issuedByName = row.issuedBy.first_name + ' ' + row.issuedBy.last_name;
                    }
                    ...
                    dataHtml += `
                        ...
                        >
                            ...
                            <td class="px-4 py-2">${fromWarehouseName}</td>
                            <td class="px-4 py-2">${issuedByName}</td>
