    ...
    <section class="bg-gray-50">
        <div class="mx-auto ">
            <div class="block md:flex justify-between mb-4 items-center">
                <h3 class="text-2xl font-medium font-sans">Item Variants</h3>
                ...
                <div class="block md:flex gap-2 items-center">
                    ...
                    <a
                        href="{{ route('inventory.item.create', ['type' => 'variant']) }}"
    ...
    <script>
        ...
        const printData = (data) => {
                ...
                data.map((row, index) => {
                    dataHtml += `
                        ...
                        >
                            ...
                            <td class="px-4 py-2">${row.group ? row.group.name : '-'}</td>
                            <td class="px-4 py-2">${row.uom ? row.uom.name : '-'}</td>
                        </tr>
                    `;
                });
            } else {
            ...
            $(`#data_table tbody tr`).each((index, elem) => {
                let editRoute = `{{ route('inventory.item.edit', ['item' => '@@@']) }}`;
                $(elem).click(() => {
                    if($(elem).data('id')) {
        ...
        const fetchData = (page = 1, q = null) => {
            ...
            $.ajax({
                url: "{{ route('inventory.item.variant.index') }}",
                type: "GET",
                data: {
                    page: page,
                    q: q
                ...
                success: function(response) {
                    printData(response.data.items.data);
