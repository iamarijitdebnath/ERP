    ...
    <section class="bg-gray-50">
        <div class="mx-auto">
            <div class="flex justify-between mb-4">
                <h3 class="text-2xl font-medium font-sans">{{ isset($itemGroup) ? 'Edit Item Group' : 'Add Item Group' }}</h3>
                <div class="flex items-center gap-2">
                    <a href="{{ route('inventory.item-group.index') }}" 
                    ...
                    @if(isset($itemGroup))
                        <form id="data_delete" action="{{ route('inventory.item-group.delete', ['itemGroup' => $itemGroup->id]) }}" method="POST" class="contents">
                            @csrf
                            @method('DELETE')
                            <button 
                                type="button"
            ...
            <div class="relative overflow-hidden bg-white border border-gray-200 p-4 sm:rounded-lg">
                <form id="item-group-form" action="{{ isset($itemGroup) ? route('inventory.item-group.update', ['itemGroup' => $itemGroup->id]) : route('inventory.item-group.store') }}" method="POST" autocomplete="off">
                    @csrf
                    @if(isset($itemGroup))
                        @method('PUT')
                    @endif
                    ...
                    <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                        ...
                        <div>
                            ...
                            <select id="is_active" name="is_active" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5">
                                <option value="1" {{ (old('is_active', $itemGroup->is_active ?? 1) == 1) ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ (old('is_active', $itemGroup->is_active ?? 1) == 0) ? 'selected' : '' }}>Inactive</option>
                            </select>
                            @error('is_active')
                                <span class="text-red-400 text-xs">{{ $message }}</span>
                            @enderror
                         ...
                         <div class="sm:col-span-2">
                            ...
                            <textarea
                                ...
                                id="description"
                                rows="3"
                                class="bg-white border text-gray-900 text-sm rounded-lg block w-full p-2.5 {{ $errors->has('description') ? 'border-red-500' : 'border-gray-300' }}"
                                placeholder="Description"
                            >{{ old('description', $itemGroup->description ?? '') }}</textarea>
                            @error('description')
                                <span class="text-red-400 text-xs">{{ $message }}</span>
                            @enderror
                        </div>
...
<div id="popup-modal" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-lg shadow border border-gray-200 p-6 text-center">
            <h3 class="mb-5 text-lg font-normal text-gray-700">Are you sure you want to delete this item group?</h3>
