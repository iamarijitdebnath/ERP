    ...
    <section class="bg-gray-50">
        <div class="mx-auto ">
            <div class="block md:flex justify-between mb-4 items-center">
                <h3 class="text-2xl font-medium font-sans">Stock Transfers</h3>
...
@push('scripts')
    <script>
        ...
        const printData = (data) => {
            ...
            if(data.length > 0) {
                data.map((row, index) => {
                    let date = new Date(row.transaction_date).toLocaleDateString();
                    ...
                    dataHtml += `
                        ...
                        >
                            ...
                            <td class="px-4 py-2">${row.from_warehouse ? row.from_warehouse.name : '-'}</td>
                            <td class="px-4 py-2">${row.to_warehouse ? row.to_warehouse.name : '-'}</td>
                            <td class="px-4 py-2">${row.items ? row.items.length : 0}</td>
                        </tr>
                    `;
                });
            } else {
