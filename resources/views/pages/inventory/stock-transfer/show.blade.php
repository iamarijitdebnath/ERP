        ...
        <div class="mx-auto">
            <div class="flex justify-between mb-4">
                <h3 class="text-2xl font-medium font-sans">
                    {{ isset($transaction) ? 'Edit Stock Transfer' : 'Create Stock Transfer' }}
                </h3>
                <div class="flex items-center gap-2">
                    ...
                    @if(isset($transaction))
                        <form id="data_delete" action="{{ route($routePrefix . '.delete', ['transaction' => $transaction->id]) }}" method="POST" class="contents">
                            @csrf
                            @method('DELETE')
                            <button 
                                type="button"
            ...
            <form id="transaction-form" action="{{ isset($transaction) ? route($routePrefix . '.update', ['transaction' => $transaction->id]) : route($routePrefix . '.store') }}" method="POST" autocomplete="off">
                <div class="relative overflow-hidden bg-white border border-gray-200 p-4 sm:rounded-lg mb-4">
                    ...
                    <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                        ...
                        <div>
                            <label for="transaction_date" class="block mb-2 text-sm font-medium text-gray-900">Transaction Date</label>
                            <input
                                type="datetime-local"
                                ...
                                <table class="w-full text-sm text-left text-gray-500" id="items-table">
                                    <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                                        <tr>
                                            ...
                                            <th scope="col" class="px-6 py-3" style="width: 15%">Lot No</th>
                                    ...
                                    <tbody>
                                        @forelse($transaction->items ?? [] as $index => $item)
                                            <tr class="bg-white border-b hover:bg-gray-50 text-sm">
                                                ...
                                                <td class="p-2">
                                                    ...
                                                    >
                                                        <option value="{{ $item->item_id }}" selected>{{ $item->item->name }} ({{ $item->item->code }})</option>
                                                    </select>
                                                ...
                                                <td class="p-2">
                                                    ...
                                                    >
                                                        <option value="{{ $item->uom_id }}" selected>{{ $item->uom->name }}</option>
                                                    </select>
                                                </td>
                                                <td class="p-2">
                                                    <input type="number" step="0.01" name="items[{{ $index }}][quantity]" value="{{ $item->quantity }}" class="w-full border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
                                                </td>
                                                <td class="p-2">
                                                ...
                                                <td class="p-2">
                                                    <input type="text" name="items[{{ $index }}][lot_no]" value="{{ $item->lot_no }}" class="w-full border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
                                                </td>
                                                <td class="p-2">
                                        ...
                                        @empty
                                            <tr class="bg-white border-b hover:bg-gray-50 text-sm">
                                                ...
                                                <td class="p-2">
                                                    <input type="text" name="items[0][lot_no]" class="w-full border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
                                                </td>
                                            </tr>
                                        @endforelse
                                    </tbody>
    ...
    <div id="popup-modal" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow border border-gray-200 p-6 text-center">
                <h3 class="mb-5 text-lg font-normal text-gray-700">Are you sure you want to delete this Transfer?</h3>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#delete_selected_btn').click(function() {
                ...
                const totalRows = $('#items-table tbody tr').length;
                
                if (selectedRows.length === 0) return;

                if (selectedRows.length === totalRows) {
                     alert("You cannot delete all rows. At least one row is required.");
                     return;
                }
            ...
            $('#row_add_btn').click(() => {
                ...
                const newRow = `
                    <tr class="bg-white border-b hover:bg-gray-50 text-sm">
                        ...
                        <td class="p-2">
                            <input type="text" name="items[${uniqueIndex}][lot_no]" class="w-full border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
                        </td>
                        <td class="p-2">
