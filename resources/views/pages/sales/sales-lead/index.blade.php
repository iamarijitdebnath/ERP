...
@section('title', 'Selling Leads')

@section('content')
    <section class="bg-gray-50">
        <div class="mx-auto">
            <div class="block md:flex justify-between mb-4 items-center">
                <h3 class="text-2xl font-medium font-sans">Selling Leads</h3>
                ...
                <div class="block md:flex gap-2 items-center"> 
                    ...
                    <a
                        href="{{ route('sales.sales-lead.create') }}"
    ...
    <script>
        ...
        const printData = (data) => {
                ...
                data.map((row, index) => {
                    dataHtml += `
                        ...
                        >
                            ...
                            <td class="px-4 py-2">${row.mobile || '-'}</td>
            ...
            $(`#data_table tbody tr`).each((index, elem) => {
                let showRoute = `{{ route('sales.sales-lead.show', ['lead' => '@@@']) }}`;
                $(elem).click(() => {
                    if($(elem).data('id')) {
        ...
        const fetchData = (page = 1, q = null) => {
            ...
            $.ajax({
                url: "{{ route('sales.sales-lead.index') }}",
                type: "GET",
                data: {
                    page: page,
                    q: q,
                ...
                success: function(response) {
                    printData(response.data.leads.data);
                    renderPagination(response.data.leads);
                },
                error: function(xhr) {
        ...
        $(`#rows_per_page`).on('change', () => {
            fetchData(1, $(`#data_search`).val());
        });

        fetchData({{ request('page', 1) }}, "{{ request('q') }}");
    </script>
@endpush
