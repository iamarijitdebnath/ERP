...
<section class="bg-gray-50">
    <div class="mx-auto ">
        <div class="block md:flex justify-between mb-4 items-center">
            <h3 class="text-2xl font-medium font-sans">Menus</h3>
                    ...
                    <tbody>
                        @foreach ($menus as $menu)
                            <tr 
                                ...
                                data-href="{{ route('system.menu.edit', $menu) }}"
                                role="link"
                                tabindex="0"
                            >
                                <td class="px-4 py-2">
                                    <div class="truncate">{{ $menu->name }}</div>
                                </td>
                                <td class="px-4 py-2">
                                    <div class="truncate">{{ $menu->route }}</div>
                                </td>
                                <td class="px-4 py-2">
                                    <div class="truncate">{{ $menu->group->name ?? 'N/A' }}</div>
                                </td>
                                <td class="px-4 py-2">
                                    <div class="truncate">{{ $menu->group->module->name ?? 'N/A' }}</div>
                                </td>
                                <td class="px-4 py-2">
                                    <span
                                        class="inline-flex items-center text-white text-sm font-medium px-3 py-1 rounded-full {{ $menu->is_active ? 'bg-blue-500' : 'bg-red-500' }}"
                                    >
